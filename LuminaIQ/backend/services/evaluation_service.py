...
Respond ONLY with the valid JSON array."""
            ...
            # 5. Store in database
            test_id = str(uuid4())
            await async_db(lambda: self.client.table("subjective_tests").insert(
                {
                    "id": test_id,
    ...
    async def submit_subjective_test(
        ...
        try:
            ...
            await async_db(lambda: self.client.table("subjective_tests").update(
...
Be constructive and specific in your feedback. Respond ONLY with the JSON object."""
            ...
            await async_db(lambda: self.client.table("answer_evaluations").insert(
                {
                    "id": evaluation_id,
                    "project_id": project_id,
                    "user_id": user_id,
