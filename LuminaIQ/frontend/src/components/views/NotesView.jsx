...
const NotesView = ({ projectId, availableTopics, selectedDocuments, preSelectedTopic = null, preGeneratedData = null, onConsumePreGenerated = null, autoGenerate = false }) => {
    ...
    useEffect(() => {
        if (preSelectedTopic && availableTopics?.includes(preSelectedTopic)) {
            ...
            // If autoGenerate is set (came from Learning Path click), skip straight to generation
            if (autoGenerate) {
                setViewMode('form');
            }
        }
    }, [preSelectedTopic, autoGenerate]);

    // Auto-trigger generation when topic + autoGenerate are both ready
    const autoGeneratedRef = useRef(false);
    useEffect(() => {
        if (autoGenerate && notesTopicSelection && notesTopicSelection !== '__custom__' && !autoGeneratedRef.current) {
            autoGeneratedRef.current = true;
            handleGenerateNotes();
        }
    }, [autoGenerate, notesTopicSelection]);
