...
class AuthService:
    def __init__(self):
        pass # We no longer store a global client here to prevent singleton session pollution

    def _get_auth_client(self):
        """Create an ephemeral client using the ANON key for auth operations so we don't pollute the global service key client"""
    ...
    async def login_with_google(self, supabase_token: str) -> Dict[str, Any]:
        ...
        try:
            auth_client = self._get_auth_client()
            user = auth_client.auth.get_user(supabase_token)
